...
public class vp_SimpleInventory : MonoBehaviour
{
	...
	public InventoryWeaponStatus CurrentWeaponStatus{
		get{ return m_CurrentWeaponStatus; }
	...
	public List<InventoryItemStatus> Weapons
	{
		...
		get{
			...
			foreach(InventoryItemStatus weapon in m_WeaponTypes)
				weapons.Add(weapon);
	...
	public List<InventoryItemStatus> EquippedWeapons
	{
		...
		get{
			List<InventoryItemStatus> weapons = new List<InventoryItemStatus>();
			foreach(InventoryItemStatus weapon in m_ItemStatusDictionary.Values)
				if(weapon.GetType() == typeof(InventoryWeaponStatus))
					if(weapon.Have == 1)
						weapons.Add(weapon);
